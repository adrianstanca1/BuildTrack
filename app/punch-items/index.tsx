...
export default function PunchItemsScreen() {
  ...
  const filteredItems = filterStatus === 'all'
    ? punchItems
    : punchItems.filter((i) => i.status === filterStatus);
      ...
      <View className="flex-1 bg-gray-50 dark:bg-gray-900">
        <View className="p-4">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-2xl font-bold text-gray-900 dark:text-white">Punch Items</Text>
            <Pressable
              onPress={() => router.push('/quick-actions/punch')}
          ...
          <FlatList
                ...
                >
                  ...
                  {item.assignee && (
                    <View className="flex-row items-center mt-2">
                      ...
                      <Text className="text-xs text-gray-500 ml-1">Assigned: {item.assignee}</Text>
                    </View>
                  )}
                </Pressable>
              </Card>
            ...
            ListEmptyComponent={
              <View className="items-center py-12">
                <Ionicons name="hammer-outline" size={48} color={colors.gray} />
                <Text className="text-gray-500 mt-4 text-center">
                  {filterStatus !== 'all'
                    ? `No ${filterStatus} punch items`
