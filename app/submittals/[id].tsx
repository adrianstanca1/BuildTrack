...
export default function SubmittalDetailScreen() {
  ...
  const { submittals, deleteSubmittal } = useSubmittalsStore();


  const item = submittals.find((e) => e.id === id);
  ...
  if (!item) {
    return (
      <SafeAreaView className="flex-1 justify-center items-center bg-gray-50">
        ...
        <Text className="text-gray-400 mt-4">Submittal not found</Text>
      </SafeAreaView>
    );
  }
      ...
      <ScrollView className="px-4 py-4">
        ...
        <Card className="mt-4">
          ...
          <View className="flex-row items-center py-2">
            ...
            <View className="ml-3 flex-1">
              <Text className="text-xs text-gray-400">Spec Section</Text>
              <Text className="text-sm font-medium text-gray-900">{item.specSection || "N/A"}</Text>
            </View>
          </View>
          <View className="flex-row items-center py-2">
            <Ionicons name="list-outline" size={20} color={isDark ? '#6b7280' : '#9ca3af'} />
          ...
          <View className="flex-row items-center py-2">
            ...
            <View className="ml-3 flex-1">
              <Text className="text-xs text-gray-400">Submitted By</Text>
              <Text className="text-sm font-medium text-gray-900">{item.submittedBy || "N/A"}</Text>
            </View>
          </View>
          <View className="flex-row items-center py-2">
            <Ionicons name="calendar-outline" size={20} color={isDark ? '#6b7280' : '#9ca3af'} />
            <View className="ml-3 flex-1">
              <Text className="text-xs text-gray-400">Submitted Date</Text>
              <Text className="text-sm font-medium text-gray-900">{item.submittedDate || "N/A"}</Text>
            </View>
          </View>
          <View className="flex-row items-center py-2">
            <Ionicons name="calendar-outline" size={20} color={isDark ? '#6b7280' : '#9ca3af'} />
